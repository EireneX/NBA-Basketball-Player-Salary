...
col_names_raw = ["player_id","Season","Tm","Pos"]
...
query = query.format(player_team, ','.join(temp_list))
print query
...
# loop through all files & insert stats into player_team table 
for i in range(0, len(file_paths)):
    with open(file_paths[i], 'r') as f:
        ...
        query = query.format(player_team, ','.join(col_names), ','.join('?' * len(col_names)))
        for row in reader:
